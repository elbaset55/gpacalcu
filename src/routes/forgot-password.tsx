...
function ForgotPasswordPage() {
  ...
  return (
    <div dir={dir} style={{ fontFamily: ar ? FONT : FONT_EN, minHeight: "100vh", background: BG, display: "flex", alignItems: "center", justifyContent: "center", padding: "20px 16px", position: "relative", overflow: "hidden" }}>
      ...
      <style>{`
        @keyframes fp-in { from { opacity:0; transform:translateY(22px) scale(0.97); } to { opacity:1; transform:translateY(0) scale(1); } }
        @keyframes fp-spin { to { transform:rotate(360deg); } }
      `}</style>

      <div style={{ width: "100%", maxWidth: 440, position: "relative", zIndex: 1 }}>
        {/* Header row */}
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 28, animation: "fp-in 0.4s cubic-bezier(0.22,1,0.36,1) both" }}>
          <Logo height={36} />
        ...
        <div style={{
          ...
          animation: "fp-in 0.55s 0.1s cubic-bezier(0.22,1,0.36,1) both",
        }}>
          {/* Icon */}
          <div style={{ width: 56, height: 56, borderRadius: 16, background: isDark ? "rgba(79,255,176,0.1)" : "rgba(32,84,224,0.08)", border: `1px solid ${isDark ? "rgba(79,255,176,0.22)" : "rgba(32,84,224,0.15)"}`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 26, margin: "0 auto 20px" }}>
            🔑
          </div>

          <div style={{ textAlign: "center", marginBottom: 24 }}>
            <h1 style={{ margin: 0, fontSize: 22, fontWeight: 900, color: TEXT, fontFamily: ar ? FONT : FONT_EN }}>
              {ar ? "استعادة الوصول" : "Account Recovery"}
            </h1>
            <p style={{ fontSize: 13, color: MUTED, margin: "8px 0 0", fontFamily: ar ? FONT : FONT_EN, lineHeight: 1.7 }}>
              {ar
                ? "Termly يستخدم تسجيل الدخول عبر Google أو Replit. لاستعادة وصولك، سجّل دخولك بنفس الحساب الذي استخدمته عند التسجيل."
                : "Termly uses Google or Replit sign-in. To recover access, sign in with the same account you used when you registered."}
            </p>
          </div>

          {/* Info box */}
          <div style={{ background: isDark ? "rgba(79,255,176,0.06)" : "rgba(32,84,224,0.05)", border: `1px solid ${isDark ? "rgba(79,255,176,0.15)" : "rgba(32,84,224,0.12)"}`, borderRadius: 12, padding: "12px 16px", marginBottom: 20 }}>
            <div style={{ fontSize: 12, color: isDark ? "rgba(79,255,176,0.85)" : ACCENT, fontFamily: ar ? FONT : FONT_EN, lineHeight: 1.6 }}>
              💡 {ar
                ? "إذا سجّلت دخولك بـ Google، فبياناتك ستُسترجع تلقائياً عند تسجيل الدخول مجدداً."
                : "If you signed up with Google, your data will be automatically restored upon signing in again."}
            </div>
          </div>
          ...
          <a href="/api/auth/login" style={{
            display: "flex", alignItems: "center", justifyContent: "center", gap: 10,
            width: "100%", padding: "13px 16px",
            background: isDark ? "linear-gradient(135deg,rgba(79,255,176,0.12),rgba(124,131,245,0.10))" : "linear-gradient(135deg,rgba(32,84,224,0.08),rgba(91,96,232,0.07))",
            ...
            <svg width={17} height={17} viewBox="0 0 24 24" fill="currentColor"><path d="M3 3h8v8H3zm10 0h8v8h-8zm0 10h8v8h-8zM3 13h8v8H3z" /></svg>
            {ar ? "متابعة بـ Replit" : "Continue with Replit"}
          </a>
        ...
        {/* Footer */}
        <p style={{ textAlign: "center", marginTop: 16, fontSize: 11, color: isDark ? "rgba(200,210,240,0.3)" : "rgba(15,23,66,0.3)", fontFamily: ar ? FONT : FONT_EN, animation: "fp-in 0.5s 0.3s both" }}>
